    ...
    if scrol_token:
        with open('dataset.json', 'r') as openfile:
            ...
            if(base_model == 'mistralai/Mistral-7B-v0.1'):
                ...
                TrainMistral7b(base_model="mistralai/Mistral-7B-v0.1", dataset_path='dataset.json', output_dir=f"{output_dir}", max_steps=max_steps, max_length=max_length, resume_checkpoint=resume_checkpoint, batch_size=batch_size, wb_token=wb_token)
                # print("uploading finetuned model to storage")
                # upload_folder("https://scrol-internal-testing.onrender.com/upload-model", output_dir, payload)
            else:
        ...
        if(user_input == 'y'):
            with open('dataset.json', 'r') as openfile:
                ...
                if(base_model == 'mistralai/Mixtral-8x7B-v0.1' or base_model=="mistralai/Mixtral-8x7B-Instruct-v0.1"):
                    ...
                    print("training mistral moe")
                    TrainMistral8x7b(base_model=base_model, dataset_file='dataset.json', output_dir=f"{output_dir}", max_steps=max_steps, max_length=max_length, resume_checkpoint=resume_checkpoint, batch_size=batch_size, wb_token=wb_token)
                    # print("uploading finetuned model to storage")
                    # upload_folder("https://scrol-internal-testing.onrender.com/upload-model", output_dir, payload)
                ...
                if(base_model == 'mistralai/Mistral-7B-v0.1' or base_model == 'mistralai/Mistral-7B-Instruct-v0.1'):
                    print('training mistral 7b model')
                    TrainMistral7b(base_model=base_model, dataset_file='dataset.json', output_dir=f"{output_dir}", max_steps=max_steps, max_length=max_length, resume_checkpoint=resume_checkpoint, batch_size=batch_size, wb_token=wb_token)
                    print("training completed.")
                    print(f"fine tuning weights are present in dir {output_dir}")
                else:
                    print(f'this model ${base_model} is not available in library.')
